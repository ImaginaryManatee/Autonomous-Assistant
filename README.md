...
Daily update 03/18
Today, I worked on creating the functions necessary for the assistant to take in a PDF file, chunk it, convert those chunks to a vector, and upload them to a database to refer to in the future. As I consider what to upload in the assistant, I had an inspiring thought. What if, instead of a secretary, the autonomous assistant could be a tour guide for parks? The idea would be that the user would be able to send pictures to through the front end and have the assistant provide information based on the picture. E.g. Identity of plant, history, whether it is edible, etc.
